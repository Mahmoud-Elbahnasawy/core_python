...
# when checking the type of any of the imported classes above it shows something like this
# refactoring_to_separate_concerns.Polygon
# so i will make a dict to hold the class name only as the class type 
# so that we check it type it shows something like this Polygon
# dict_of_net_types = {
# "refactoring_to_separate_concerns.Polygon": Polygon , 
# "refactoring_to_separate_concerns.Circle": Circle , 
# "refactoring_to_separate_concerns.Rectangle": Rectangle , 
# "refactoring_to_separate_concerns.Group": Group
# }

dict_of_drawers = {
    Circle:draw_circle,
    Rectangle:draw_rectangle,
    Polygon:draw_polygone,
    Group:draw_group
}
def draw(shape):

    type_of_shape = type(shape)
    print(colored(f"the type of the passed shape is {type_of_shape}, while the shape is {shape}", color='red'))

    try :
        # print(f"the type of the passed shape is {type_of_shape}")
        caller_reference = dict_of_drawers[type_of_shape]
        # print(f"the reference is {caller_reference}")
    except :
        raise TypeError(f"shape type is not supported {type_of_shape}!.")
    else:
